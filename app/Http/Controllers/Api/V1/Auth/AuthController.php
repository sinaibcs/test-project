...
class AuthController extends Controller
{
    ...
    public function forgotPassword(Request $request)
    {
        ...
        $data = $this->authService->AdminForgotPassword($user);
        OtpLog::create(['user_id' => $user->id, 'phone_number' => $user->mobile, 'type' => 'FORGOT_PASSWORD', 'body' => $data]);
...
//                'device' => $mode,
    ...
    public function BrowserRegisterOtp(Request $request)
    {
        // $device = Device::where("device_name", 'test')->first(); // remove during Live - this is only for testing purpose
        $mode = $request->username;
        ...
        $this->authService->validateLogin($request); //Keep in Live
        //  login
        $data = $this->authService->BrowserRegisterOtp($request, 1);
...
//                'device' => $mode,
    ...
    {
        $data = $this->authService->sendGeneralOtp($request->user());
