    ...
    public function createUser(Request $request, $password)
    {
        DB::beginTransaction();
         try {
            ...
            $user->mobile = Helper::banglaToEnglish($request->mobile);
            $user->email = $request->email;
             ...
             if($request->file('image_signature')){
                 $image_signature = $request->file('image_signature');
                 $destinationPath = env('FILE_STORE_ROOT').'/user/profile-images';
                 $imageUrl = Helper::uploadImage($image_signature, $destinationPath);
                 $user->photo_signature = $imageUrl;
             }
...
//            $user->status = $request->status;
            ...
            if ($request->office_type == 8) {
                $this->saveUserUnions($user, $request);
            }
    ...
    public function saveUserUnions($user, $request)
    {
        $unions = $request->office_union_id ? explode(',', $request->office_union_id) : [];

        $user->unions()->sync($unions);
